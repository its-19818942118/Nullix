...
fn_status() {
    ...
    "Full")
        ...
        ;;
    *)
        if [[ ! -f "/tmp/Helix.batterynotify.status.fallback.$battery_status-$$" ]]; then
            echo "Status: '==>> ""$battery_status"" <<==' Script on Fallback mode,Unknown power supply status. Please copy this line and raise an issue to the Github Repo. Also run 'ls /tmp/Helix.batterynotify' to see the list of lock files.*"
            touch "/tmp/Helix.batterynotify.status.fallback.$battery_status-$$"
        fi
        fn_percentage
        ;;
    esac
...
get_battery_info() {
    ...
    for battery in /sys/class/power_supply/BAT*; do
        battery_status=$(grep -w "POWER_SUPPLY_STATUS" "$battery/uevent" | cut -d= -f2)
        battery_percentage=$(grep -w "POWER_SUPPLY_CAPACITY" "$battery/uevent" | cut -d= -f2)
...
# main function
main() {
    rm -fr /tmp/Helix.batterynotify* # cleaning the lock file
...
case "$1" in
-m | --modify)
    EDITOR="${EDITOR:-code-oss}" #* Use VS Code as the default editor
