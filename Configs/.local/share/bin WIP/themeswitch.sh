...
[ -z "${HelixTheme}" ] && echo "ERROR: unable to detect theme" && exit 1
get_themes
...
set_conf "HelixTheme" "${themeSet}"
echo ":: applying theme :: \"${themeSet}\""
export reload_flag=1
source "${scrDir}/globalcontrol.sh"

# hypr
sed '1d' "${HelixThemeDir}/hypr.theme" > "${confDir}/hypr/themes/theme.conf"
gtkTheme="$(
{ grep -q "^[[:space:]]*\$GTK-THEME\s*=" "${HelixThemeDir}/hypr.theme" && grep "^[[:space:]]*\$GTK-THEME\s*=" "${HelixThemeDir}/hypr.theme" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} || 
grep 'gsettings set org.gnome.desktop.interface gtk-theme' "${HelixThemeDir}/hypr.theme" | awk -F "'" '{print $((NF - 1))}'
)"
gtkIcon="$(
{ grep -q "^[[:space:]]*\$ICON-THEME\s*=" "${HelixThemeDir}/hypr.theme" && grep "^[[:space:]]*\$ICON-THEME\s*=" "${HelixThemeDir}/hypr.theme" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} ||  
grep 'gsettings set org.gnome.desktop.interface icon-theme' "${HelixThemeDir}/hypr.theme" | awk -F "'" '{print $((NF - 1))}'
)"
...
# wallpaper
"${scrDir}/swwwallpaper.sh" -s "$(readlink "${HelixThemeDir}/wall.set")"