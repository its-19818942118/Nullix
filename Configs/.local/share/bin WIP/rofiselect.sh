...
rofiAssetDir="$HOME/.local/share/bin/Helix/rofi/assets"
...
# launch rofi menu
RofiSel=$(find "${rofiStyleDir}" -name "style_*.rasi" -print0 | xargs -0 -I{} basename {} | awk -F '[_.]' '{print $((NF - 1))}' | while read styleNum
do
