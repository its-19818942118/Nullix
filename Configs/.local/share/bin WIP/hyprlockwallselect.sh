...
# launch rofi menu
currentWall="$(basename "$(readlink "${HelixThemeDir}/wall.set")")"
...
if [ -n "${rofiSel}" ] ; then
    ...
    "${scrDir}/hyprlockwallpaper.sh" -s "${setWall}"
    notify-send -a "t1" -i "${thmbDir}/$(set_hash "${setWall}").sqre" "Set Hyprlock Wallpaper to ${rofiSel}"
fi